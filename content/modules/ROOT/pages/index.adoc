...
NOTE: While traditionally you'll be prompted to configure options and download the taxonomy, here you can just hit kbd:[ENTER] for the default settings.

Press `Y` to clone the taxonomy repository, and continue to hit kbd:[ENTER] for the remaining default settings:
...
The last step of the ilab configuration is to select a train profile. Train profiles are GPU specific profiles that enable accelerated training behavior. There are various flags you can utilize with individual ilab commands that will allow you to utilize your GPU if applicable. **For this lab**, we are using a single NVIDIA L4 GPU. We will not be performing any training due to time and resource constraints, therefore our selection is inconsiquential.

Select the **No Profile (CPU-only)** option at the following prompt:
...
NOTE: When prompted to enter a training profile, enter 0 as shown in the above output.
...
With the InstructLab environment configured, you will now download two different quantized (compressed and optimized) models to your local directory. Granite will be used as a model server for API requests, and Merlinite will help train a new model.
...
The output after each download command should resemblethe following:
...
[source,console,role=execute,subs=attributes+]
----
cp -av ~/files/qna.yaml /home/instruct/.local/share/instructlab/taxonomy/knowledge/instructlab/overview
----
...
In the **upper** terminal window, ensure that the Granite model is no longer deployed by hitting kbd:[CTRL+C]:
...
----
cd ~/instructlab
ilab model serve --model-path /home/instruct/.cache/instructlab/models/merlinite-7b-lab-Q4_K_M.gguf
----

We will now run the command (in the second, **bottom** Terminal) to generate the synthetic data:
...
InstructLab is now synthetically generating data based on the seed data you provided in the `qna.yaml` file. This will take a few minutes. 
...
NOTE: Training using the newly generated data is a time and resource intensive task. Depending on many factors, it can take from 20 minutes up to an hour and is highly dependent on the hardware used. We will not train the model during this lab as we will use an already tuned model.
...
NOTE: Make sure to stop the previous `ilab model serve` command that may still be running in the other terminal tab by typing kbd:[CTRL+C]

Type the below command in either terminal window:

[source,console,role=execute,subs=attributes+]
----
cd ~/instructlab
ilab model serve --model-path files/ggml-ilab-pretrained-Q4_K_M.gguf
----
